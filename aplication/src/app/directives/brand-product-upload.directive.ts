...
export class BrandProductUploadDirective  implements AfterViewInit, OnChanges{
  ...
  upload() {
    ...
    const query = `?query={"$and":[${this.cityLinkArr.length > 0 ? JSON.stringify( {$or: this.cityLinkArr} ) : {} },{"brand":"${this.brand}"}${this.filter ? this.filter : ''}]}&skip=${this.skip * 5}&limit=5&sort=${this.sortFilter}`;
