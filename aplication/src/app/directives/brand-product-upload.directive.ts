...
export class BrandProductUploadDirective  implements AfterViewInit, OnChanges{
  ...
  ngOnChanges() {
    ...
    this.skip = 1;
    const query = `/count?query={"$and":[${this.cityLinkArr.length > 0 ? JSON.stringify( {$or: this.cityLinkArr} ) : {} },{"brand":"${this.brand}"}${this.filter ? this.filter : ''}]}`;
  ...
  upload() {
    ...
    const query = `?query={"$and":[${this.cityLinkArr.length > 0 ? JSON.stringify( {$or: this.cityLinkArr} ) : {} },{"brand":"${this.brand}"}${this.filter ? this.filter : ''}]}&populate={"path":"companyOwner"}&skip=${this.skip * 5}&limit=5&sort=${this.sortFilter}`;
